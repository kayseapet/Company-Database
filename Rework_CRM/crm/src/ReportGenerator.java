...
public class ReportGenerator {
    ...
    public void generateEmployeePayHistoryReport(int empId) {
        ...
        String empSql = "SELECT * FROM employee WHERE emp_id = ?";
        String paySql = "SELECT * FROM pay_statement WHERE emp_id = ? ORDER BY start_date DESC";
            ...
            try (PreparedStatement payStmt = conn.prepareStatement(paySql)) {
                ...
                while (payRs.next()) {
                    ...
                    System.out.printf("%-12s $%-11.2f $%-11.2f $%-11.2f\n",
                            payRs.getDate("start_date"), gross, deductions, net);
    ...
    public void generateMonthlyPayByJobTitle(int year, int month) {
        ...
        // Corrected SQL for Monthly Pay
        String sql = """
            SELECT e.job_title, SUM(ps.amount) as total_pay, COUNT(ps.id) as pay_count
            FROM employee e
            JOIN pay_statement ps ON e.emp_id = ps.emp_id
            WHERE YEAR(ps.start_date) = ? AND MONTH(ps.start_date) = ?
    ...
    public void generateMonthlyPayByDivision(int year, int month) {
        ...
        String sql = """
                    SELECT e.division, SUM(ps.gross_pay) as total_pay, COUNT(ps.id) as pay_count
                    FROM employee e
                    JOIN pay_statement ps ON e.emp_id = ps.emp_id
