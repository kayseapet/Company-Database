...
public class InputHandler {
    ...
    public void requestSearchEmployee() {
        ...
        boolean colExists = SchemaManager.doesColExist(searchCol);
        while (!colExists) {
            System.out.println("Column does not exist. Please try again.");
            searchCol = Main.requestString("Enter search term (name, job title, etc.): ");
            colExists = SchemaManager.doesColExist(searchCol);
        }
        String searchValue = Main.requestString("Enter value to search for: ");
        try{
            ResultSet rs = employeeManager.searchEmployee(searchCol, searchValue);
            employeeManager.employeeDAO.printResults(rs);
            if (rs == null || !rs.next()) {
                System.out.println("No employees found with " + searchCol + " = " + searchValue);
            }
        } catch (SQLException e) {
